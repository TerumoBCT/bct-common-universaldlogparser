...
namespace Bct.Common.LogParsing.Sinks
{
   ...
   /// 
   /// </summary>
   public sealed class DlogInputReaderSink : IDlogParsingInputReaderSink
   {
      /// <summary>
      /// 
      /// </summary>
      ...
      public Enums.ParsingResult Read()
      {
         ...
         try
         {
            using (FileStream inFile = File.Open(inputFileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
               if (ReadUntilHeaderRecord(inFile, ref dlogHelper) >= 0)
               {
                  ...
                  dlogHelper.DeviceSerialNumber = serialNumber;
                  using (GZipStream decompressStream = new GZipStream(inFile, CompressionMode.Decompress))
                  {
                     do
                     {
          ...
          ref DataLogReaderHelper oDlogStart)
      {
         ...
         do
         {
            nRead = inFile.Read(readBuffer, 0, readBuffer.Length);
            if (nRead > 0)
            {
               if (bofBuffer == null)
               {
               ...
               if (nStartDataOffset != 0)
               {
                  ...
                  inFile.Seek(nStartDataOffset + sizeof(Int32), SeekOrigin.Begin);
